...
## Design System
UX template → `_shared/design-system/design-tokens.css` (OKLCH colors, light/dark mode). Game HUD → `_shared/design-system/game-tokens.css`. Typed tokens → `_shared/design-system/design-tokens.ts`. 5 game palettes (VIBRANT/PASTEL/DARK/NEON/RETRO) → `skills/pxh-expert/games-2d/templates/color-palettes.ts`.

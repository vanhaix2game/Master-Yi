export function buildIR(params) {
    ...
    const hasCritical = params.constraints.some(c => ['preserve_behavior', 'no_breaking_changes', 'security_first', 'no_hallucination'].includes(c));
    const hasHighPriorityIntent = params.intents.some(i => ['rapid_prototype', 'integrate_systems'].includes(i));
    return {
        version: '1.0',
        raw: params.raw,
...
function extractFrameworks(normalized, lexemes) {
...
function extractLanguages(normalized, lexemes) {
...
function extractPlatforms(normalized, lexemes) {
...
function extractLibraries(normalized, lexemes) {
...
function extractFiles(input, lexemes) {
    ...
    return [...fileSet].map(path => ({ path, action: 'analyze' }));
}
function detectProjectType(target) {
    if (target.frameworks.some(f => /phaser|three\.js|unity|godot/i.test(f)))
        return 'game';
    if (target.frameworks.some(f => /react|vue|angular|svelte|next/i.test(f)))
        return 'web';
    if (target.libraries.some(l => /langchain|tensorflow|pytorch/i.test(l)))
        return 'ai';
    if (target.platforms.some(p => /android|ios/i.test(p)))
        return 'mobile';
    if (target.frameworks.some(f => /electron|tauri/i.test(f)))
        return 'desktop';
    if (target.frameworks.some(f => /express|fastify|django|fastapi|spring/i.test(f)))
        return 'api';
    return undefined;
}
//# sourceMappingURL=11-ir-builder.js.map
import type { Token, Lexeme, StageMetric } from '../types.js';
...
export function lex(tokens: Token[]): { lexemes: Lexeme[]; metric: StageMetric } {
  ...
  for (const token of tokens) {
    ...
    if (trieResult.found && trieResult.output) {
      ...
      if (technicalIsFramework(techLower)) category = 'framework';
      else if (technicalIsLanguage(techLower)) category = 'language';
      else if (technicalIsPlatform(techLower)) category = 'platform';
      else if (technicalIsLibrary(techLower)) category = 'library';
      else category = 'technical_term';
    ...
    lexemes.push({ token, category: category as any, confidence });
  }
...
function technicalIsFramework(lower: string): boolean {
...
function technicalIsLibrary(lower: string): boolean {

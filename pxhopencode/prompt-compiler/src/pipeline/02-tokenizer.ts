import type { Token, StageMetric } from '../types.js';
...
function classifyWord(word: string): string {
  if (COMMAND.test(word)) return 'command';
  if (FILE_EXT.test(word)) return 'file_extension';
  if (FILE_PATH.test(word)) return 'path';
  if (IDENTIFIER.test(word)) return 'identifier';
  if (NUMBER.test(word)) return 'number';
  if (STRING.test(word)) return 'string';
  return 'unknown';
}

export function tokenize(input: string): { tokens: Token[]; metric: StageMetric } {
  ...
  while (i < input.length) {
    ...
    if (isCJK(input[i]) || /[a-zA-Z0-9_$]/.test(input[i])) {
      const start = i;
      while (i < input.length && (isCJK(input[i]) || /[a-zA-Z0-9_$]/.test(input[i]))) i++;
      const word = input.slice(start, i);
      const type = classifyWord(word) as any;

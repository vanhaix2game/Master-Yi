...
export function applyRules(input: string): { output: string; removals: number; metric: StageMetric } {
...
export function applyCleanupRules(input: string): { output: string; metric: StageMetric } {
  const t0 = performance.now();
  let output = input;
